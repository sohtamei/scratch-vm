...
class Scratch3Blocks {
	...
	constructor (runtime) {
		...
		this.charGetBatteryVoltage = null;
		this.charSetMotorPWM = null;
		this.charSetPortOut = null;
		this.charSetServoPosition = null;
		this.charBurstCommand = null;
		this.charGetFunctions = null;
	...
	_openBle() {
		const _this = this;
		let options = {
			filters: [{namePrefix: 'bCore'}],
		//	acceptAllDevices: true,
			optionalServices: [
				BLEUUID.GenericAccessService,
				BLEUUID.DeviceInformationService,
				BLEUUID.bCoreService
			]
		};
		let _service = null;
		...
		.then(gatt => {
			_this.ble = gatt;
/*
			return _this.ble.getPrimaryService(BLEUUID.GenericAccessService);
		}).then(service => {
			return _this.ble.getPrimaryService(BLEUUID.DeviceInformationService);
		}).then(service => {
*/
			return _this.ble.getPrimaryService(BLEUUID.bCoreService);
		}).then(service => {
			_service = service;
			return _service.getCharacteristic(BLEUUID.GetBatteryVoltage);
		}).then(char => {
			_this.charGetBatteryVoltage = char;
			return _service.getCharacteristic(BLEUUID.SetMotorPWM);
		}).then(char => {
			_this.charSetMotorPWM = char;
			return _service.getCharacteristic(BLEUUID.SetPortOut);
		}).then(char => {
			_this.charSetPortOut = char;
			return _service.getCharacteristic(BLEUUID.SetServoPosition);
		}).then(char => {
			_this.charSetServoPosition = char;
			return _service.getCharacteristic(BLEUUID.BurstCommand);
		}).then(char => {
			_this.charBurstCommand = char;
			return _service.getCharacteristic(BLEUUID.GetFunctions);
		}).then(char => {
			_this.charGetFunctions = char;
