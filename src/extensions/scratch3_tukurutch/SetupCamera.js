...
class Scratch3Blocks {
	...
	constructor (runtime) {
		...
		this.comlib.ifType = 'UART';

		this.Camera_name = 'unknown';

		const cookies_get = document.cookie.split(';');
		for(let i = 0; i < cookies_get.length; i++) {
			const tmp = cookies_get[i].trim().split('=');
			if(tmp[0] == 'Camera_name') this.Camera_name = tmp[1];
		}
		console.log('Camera:'+this.Camera_name+','+this.comlib.ipCamera);
			...
			for(let i = 0; i < devices.length; i++) {
				if (devices[i].kind == 'videoinput') {
					if(devices[i].deviceId == '') {
						_this.cameras.push('Built-in camera');
					} else {
						_this.cameras.push(devices[i].label);
					}
				}
			}
			if(_this.Camera_name != 'esp32camera') 
				_this._runtime.ioDevices.video.enableVideo();
		});
	}
	...
	get_blocks() {
		this._blocks = [
{blockType: BlockType.COMMAND, opcode: 'setConfig', text: 'select [ARG1] ip [ARG2]',
arguments: {
	ARG1: {type:ArgumentType.STRING, defaultValue: this.Camera_name, menu: 'cameras'},
	ARG2: {type:ArgumentType.STRING, defaultValue: (this.comlib.ipCamera =='' ? ' ' :this.comlib.ipCamera)},
}},
...
{blockType: BlockType.COMMAND, opcode: '_setConfig', text: '［esp32］'+['con/discon','接続/切断'][this._locale], arguments: {
}},

{blockType: BlockType.COMMAND, opcode: 'connectWifi', text: '［esp32］'+'ssid[ARG1] pass[ARG2]', arguments: {
	ARG1: { type: ArgumentType.STRING, defaultValue: this.comlib.ssid },
	ARG2: { type: ArgumentType.STRING, defaultValue: ' ' },
}},

{blockType: BlockType.REPORTER, opcode: 'statusWifi', text: '［esp32］'+['WiFi status','WiFi接続状態'][this._locale], disableMonitor:true, arguments: {
}},
...
cameras: { acceptReporters: true, items: '_getCameraMenu'}
	...
	setConfig(args) {
		...
		if(this.Camera_name != name || this.comlib.ipCamera != ip) {
			...
			if(name == 'esp32camera') this.comlib.ipCamera = ip;
			return;
		}
	...
	_setConfig(args) {
		return this.comlib.setConfig('UART', this.comlib.ipCamera);
	}
