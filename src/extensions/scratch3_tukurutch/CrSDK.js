...
*/
	constructor (runtime) {
		...
		this._penSkinId = this._runtime.renderer.createPenSkin();
		this._penDrawableId = this._runtime.renderer.createDrawable(StageLayering.PEN_LAYER);
		this._runtime.renderer.updateDrawableSkinId(this._penDrawableId, this._penSkinId);
...
{blockType: BlockType.REPORTER, opcode: 'getFaceX', text: 'face X', arguments: {
}},
...
paramXY: { acceptReporters: true, items: [
	...
	updateProps() {
		...
		.then(result => {
			console.log(result);
			_this.paramItems_ = result.propList;
	...
	clearStage(args) {
		this._runtime.renderer.penClear(this._penSkinId);
	}
				...
				//	case "Magnifier_Position":
					case "FocusFrameInfo":
						...
						for(let i = 0; i < info.length; i++)
							_this.drawRect(info[i][6]*1, info[i][7]*1, info[i][2]*1, info[i][3]*1, colorGreen);	// x,y,w,h
						break;
					}
				}
				return;
	...
	//	console.log(x0 + ',' + x1 + ',' + y0 + ',' + y1);	// debug
		this._runtime.renderer.penLine(this._penSkinId, attr, x0, y0, x1, y0);
		this._runtime.renderer.penLine(this._penSkinId, attr, x0, y0, x0, y1);
		this._runtime.renderer.penLine(this._penSkinId, attr, x0, y1, x1, y1);
		this._runtime.renderer.penLine(this._penSkinId, attr, x1, y0, x1, y1);
	}
