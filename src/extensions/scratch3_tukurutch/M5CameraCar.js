...
/**
 * Class for the M5CameraCar block in Scratch 3.0.
 * @constructor
 */
class Scratch3M5CameraCarBlocks {
    constructor (runtime) {
        /**
		...
		for(let i=0;i<cookies_get.length;i++) {
			...
			if(tmp[0]=='M5CameraCar_ip') {
				this._ipadrs=tmp[1];
				console.log('M5CameraCar_ip='+this._ipadrs);
				break;
			}
		}
    }
...
getCal(args,util) { return this.getTest(arguments.callee.name, args); }
    ...
    setIotIp (args) {
        ...
        document.cookie = 'M5CameraCar_ip=' + this._ipadrs + '; samesite=lax; expires=Tue, 31-Dec-2037 00:00:00 GMT;';
        if(SupportCamera) {
...
module.exports = Scratch3M5CameraCarBlocks;
