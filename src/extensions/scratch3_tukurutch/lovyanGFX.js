...
drawJpg(args,util) { return this.sendRecv('drawJpg', args); }
	...
	getLcdConfig(args, util) {
		...
		.then(result => {
			...
			targetBlock = Blockly.getMainWorkspace().getBlockById(extName+'_drawStage2');
			if(targetBlock) {
				targetBlock.childBlocks_[5].inputList[0].fieldRow[0].setValue(_this.width);
				targetBlock.childBlocks_[6].inputList[0].fieldRow[0].setValue(_this.height);
			}
	...
	drawStage(args,util) {
		const tmpData = this.comlib._runtime.renderer.drawWithMask(util.sequencer.runtime.ioDevices.video._skinId,
								[-240,240], [-180,180], {width:this.width,height:this.height}, 0);
		const tmpData2 = Base64Util.base64ToUint8Array(tmpData);
