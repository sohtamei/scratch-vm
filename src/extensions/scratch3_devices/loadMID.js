	...
	getInfo () {
		...
		return {
			...
			blocks: [
				...
				{blockType: BlockType.REPORTER, opcode: 'dataMID', text: 'dataMID [ARG1][ARG2]', arguments: {
		...
		return new Promise((resolve, reject) => {
			...
			uploadWindow.document.getElementById("upload-button").onclick = function() {
				...
				//	console.log(outbuf16);
					...
					if(targetBlock) {
						targetBlock.childBlocks_[0].inputList[0].fieldRow[0].setValue(files.item(0).name.replace('.mid',''));
						targetBlock.childBlocks_[1].inputList[0].fieldRow[0].setValue(_this._dumpBuf(outbuf16));
					}
